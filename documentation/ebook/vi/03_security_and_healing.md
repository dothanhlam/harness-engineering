...
* **Vòng lặp Tự sửa lỗi QA**: Nếu `go test` thất bại, pipeline sẽ nắm bắt chính xác các bản ghi biên dịch hoặc kiểm tra lỗi từ `workspace/qa_error.log` và phản hồi lại cho Developer agent. Agent có 3 nỗ lực để tự sửa mã của mình.
* **Vòng lặp Ủy quyền (Delegation Loop)**: Điều gì xảy ra nếu Developer agent thất bại 3 lần? Thay vì sập hệ thống, Harness ủy quyền sự thất bại đó *ngược lại lên trên*. Nó kích hoạt giai đoạn `BA_REFACTOR`, đánh thức BA agent. BA agent sẽ phân tích các bản ghi lỗi và viết lại `definitions_of_done.md` để làm rõ các điểm mơ hồ, đảm bảo lập trình viên có cơ hội tốt hơn ở chu kỳ tiếp theo.
